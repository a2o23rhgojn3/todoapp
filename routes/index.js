...
router.get('/', async function(req, res, next){
  ...
  let sql = `select *,datetime(finished, '+9 hours') from todo
              where user_id=${req.session.login.id} and checked = 0
              and finished > CURRENT_TIMESTAMP order by finished asc limit 10`;
  let records = await dball.getAllRows(sql);
  let sql2 = `select *,datetime(finished, '+9 hours') from todo
              where user_id=${req.session.login.id} and checked = 0
              and finished < CURRENT_TIMESTAMP order by finished desc limit 10`;
